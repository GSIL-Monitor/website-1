(function (root, factory) {
    ...
    function photoClip(container, option) {
        ...
        $file.on("change", function () {
            ...
            if (!/image\/\w+/.test(files.type)) {
                ...
                fileReader.onprogress = function (e) {
                    console.log('加载进度:' + (e.loaded / e.total * 100).toFixed() + "%");
                };
                fileReader.onload = function (e) {
                    lrz(files, {
                        quality: quality
                    }).then(function (rst) {
                        doFinish(startTimestamp, files.size, rst);
        ...
        function doFinish(startTimestamp, sSize, rst) {
            ...
            console.log('压缩耗时： ' + elapsedTime + 'ms')
            var sourceSizeKb = imgSize(sSize / 1024),
                sourceSizeMb = imgSize(sSize / 1024 / 1024),
                resultSizeKb = imgSize(rst.base64Len / 1024),
                resultSizeMb = imgSize(rst.base64Len / 1024/1024),
                scale = parseInt(100 - (resultSizeKb / sourceSizeKb * 100));
            console.log(rst.base64Len)
            console.log(resultSizeKb)
            console.log('源文件：' + sourceSizeKb + 'KB ≈ ' + sourceSizeMb+'mb');
            console.log('压缩后传输Base64：' + resultSizeKb + 'kb ≈ '+resultSizeMb+'mb');
            console.log('省' + scale + '%');
